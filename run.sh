...
INSTALL_DEPENDENCIES=$(pip install --upgrade -r $WERCKER_SOURCE_DIR/requirements.txt 2>&1)
if [ $? -ne 0 ]; then
...
if [ -z "$WERCKER_AWS_ECS_SERVICE_NAME" ]; then
  python "$WERCKER_SOURCE_DIR/main.py" \
    ...
    --task-definition-file "$WERCKER_SOURCE_DIR/$WERCKER_AWS_ECS_TASK_DEFINITION_FILE"
else
  python "$WERCKER_SOURCE_DIR/main.py" \
    --key "$WERCKER_AWS_ECS_KEY" \
    --secret "$WERCKER_AWS_ECS_SECRET" \
